...
if Z == 1 and lcompflag == 0:
    ...
    aPsi2n = pow(abs(xx*aPsi(xx,l+1,l)),2)
    aPsi2n = aPsi2n/sum(aPsi2n)
    plt.plot(xx,aPsi2n,linestyle='--',color='k',linewidth=4,label=apsilab)
...
if lcompflag == 0:    
    ...
    coeffdot = vr[:,minidx[0]]*basisMatrix
    #print coeffdot
    superpos = sum(coeffdot,axis=1)
    #print superpos
    spsqrd = pow(abs(superpos),2)
    normsp2 = spsqrd/sum(spsqrd)
    eigenvalue = float(real(w[minidx[0]]))
    calclabelstr = r'$E_{%i_c} = %.4f$, ' % (nn,eigenvalue)
    anaeig = (-HtGS*Z**2)/nn**2
    analabelstr = r'$E_{%i_a} = %.4f$' % (nn,anaeig)
    labelstr = calclabelstr+analabelstr
